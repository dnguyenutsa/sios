...
class Enforcer(object):
    ...
    def load_rules(self):
        ...
        LOG.debug(_('Loaded %(rule_type)spolicy rules: %(text_rules)s') %locals())
    ...
    def enforce(self, context, action, target):
        """Verifies that the action is valid on the target in this context.
            ...
            :returns: A non-False value if access is allowed.
        """
        return self._check(context, action, target, exception.Forbidden, action=action)
...
class ImageFactoryProxy(glance.domain.proxy.ImageFactory):
    ...
    def __init__(self, image_factory, context, policy):
        ...
        super(ImageFactoryProxy, self).__init__(image_factory, proxy_class=ImageProxy, proxy_kwargs=proxy_kwargs)
